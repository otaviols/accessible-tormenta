---
title: "Centurião"
book: "Ameaças de Arton"
chapter: "Capítulo 1: Ameaças"
section: "Império de Tauron"
prev: "01-arqueiro-escravo"
next: "03-furia-tauron"
---

# Centurião

> "O exército mais poderoso de Arton? Esses dias são coisa distante, amigão!"
> 
> — Selginn Lan'ze, osteon bardo

![Ilustração de Centurião](../../_imagens/ameacas-arton/centuriao-legionario.jpg)
*Descrição: Um minotauro maior e mais orgulhoso, ostentando um manto tinto sobre os ombros largos e uma crista emplumada no elmo de bronze. Quando ergue o gládio para ordenar o ataque, fica claro que não pretende apenas liderar, mas também lutar.*

Ainda que as legiões pareçam formadas por soldados absolutamente iguais, com as mesmas armaduras, lanças e escudos, um deles se adianta. Um minotauro ainda maior e mais orgulhoso, ostentando um manto tinto sobre os ombros largos e uma crista emplumada no elmo de bronze. Quando ergue o gládio para ordenar o ataque, fica claro que não pretende apenas liderar, mas também lutar.

O centurião é um comandante de legionários, mas também um valoroso guerreiro de elite, que luta nas linhas de frentes. Embora se beneficie das táticas tapistanas de combate em equipe, é também um lutador solo habilidoso. Astuto e experiente, logo reconhece o adversário mais perigoso, orientando os companheiros para neutralizá-lo — ou tomando a tarefa para si mesmo.

Diferente de muitos legionários, quase nenhum centurião abandonou sua pátria. Atuam em posições importantes, como capitães ou guarda-costas, gozando de privilégios e (quase sempre) fazendo por merecê-los. Será raro encontrá-los longe das terras imperiais, exceto liderando equipes em missões de grande importância.

---

## Centurião

### Ficha de Jogo

**ND 3**

**Humanoide (minotauro) Médio**

| Atributo | Valor |
|----------|-------|
| **Iniciativa** | +4 |
| **Percepção** | +2 |
| **Especiais** | Faro |

| Defesa | Valor |
|--------|-------|
| **Defesa** | 23 |
| **Fortitude** | +15 |
| **Reflexos** | +4 |
| **Vontade** | +8 |

| Pontos de Vida | Valor |
|----------------|-------|
| **PV** | 100 |

| Deslocamento | Valor |
|--------------|-------|
| **Caminhando** | 6m (4q) |

**Ataques**

**Corpo a Corpo**
- Gládio +14 (1d6+5, 19/x3) e
- Chifres +14 (1d6+5)

**À Distância**
- Azagaia +10 (1d6+5)

**Habilidades Especiais**

- **Coordenar Ataque (Movimento)** O centurião coordena os ataques de uma falange da qual esteja participando. Até o próximo turno do centurião, sempre que um participante da falange fizer um ataque, pode rolar dois dados e usar o melhor resultado.

- **Falange (Movimento)** Se o centurião estiver usando um escudo e adjacente a um aliado com esta habilidade, pode formar uma falange com esse aliado. Enquanto estiverem adjacentes um ao outro, os participantes da falange recebem +2 na Defesa e em testes de resistência.

- **Ordens (Movimento)** O centurião grita ordens para seus aliados em alcance médio. Eles recebem +2 em testes de perícia até o fim da cena.

**Atributos**

| For | Des | Con | Int | Sab | Car |
|-----|-----|-----|-----|-----|-----|
| 5 | 0 | 4 | 2 | 0 | 0 |

**Perícias:** Atletismo +10, Guerra +7.

**Equipamento:** Azagaia x3, escudo pesado, gládio certeiro, loriga segmentada.

**Tesouro:** Padrão.

---

## Centurião de Elite

### Ficha de Jogo

**ND 7**

**Humanoide (minotauro) Médio**

| Atributo | Valor |
|----------|-------|
| **Iniciativa** | +9 |
| **Percepção** | +9 |
| **Especiais** | Faro |

| Defesa | Valor |
|--------|-------|
| **Defesa** | 32 |
| **Fortitude** | +20 |
| **Reflexos** | +8 |
| **Vontade** | +13 |
| **Especiais** | Imunidade a encantamento, RD 5 |

| Pontos de Vida | Valor |
|----------------|-------|
| **PV** | 250 |

| Deslocamento | Valor |
|--------------|-------|
| **Caminhando** | 6m (4q) |

**Ataques**

**Corpo a Corpo**
- Gládio +23 (2d8+16, 19/x3) e
- Chifres +23 (2d6+16)

**À Distância**
- Azagaia +23 (1d6+14)

**Habilidades Especiais**

- **Coordenar Ataque (Movimento)** O centurião de elite coordena os ataques de uma falange da qual esteja participando. Até o próximo turno do centurião, sempre que um participante da falange fizer um ataque, pode rolar dois dados e usar o melhor resultado.

- **É Proibido Morrer** Aliados em alcance médio do centurião recebem RD 5.

- **Falange (Movimento)** Se o centurião estiver usando um escudo e adjacente a um aliado com esta habilidade, pode formar uma falange com esse aliado. Enquanto estiverem adjacentes um ao outro, os participantes da falange recebem +2 na Defesa e em testes de resistência.

- **Ordens (Movimento)** O centurião grita ordens para seus aliados em alcance médio. Eles recebem +2 em testes de perícia até o fim da cena.

**Atributos**

| For | Des | Con | Int | Sab | Car |
|-----|-----|-----|-----|-----|-----|
| 6 | 0 | 5 | 3 | 0 | 1 |

**Perícias:** Atletismo +15, Guerra +12.

**Equipamento:** Azagaia x3, escudo pesado, gládio certeiro, loriga segmentada reforçada.

**Tesouro:** Dobro.

---

## Medo de Altura

Todos os minotauros possuem um temor instintivo de alturas. Enquanto estiver adjacente a uma queda de 3m ou mais de altura (como um buraco ou penhasco), o minotauro fica abalado.

---

**Navegação:**
- [← Arqueiro Escravo](01-arqueiro-escravo.md)
- [Fúria de Tauron →](03-furia-tauron.md)
- [↑ Capítulo 1](../README.md)
