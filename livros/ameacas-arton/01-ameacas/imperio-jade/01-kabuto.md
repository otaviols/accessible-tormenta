---
title: "Kabuto"
book: "Ameaças de Arton"
chapter: "Capítulo 1: Ameaças"
section: "Império de Jade"
prev: "README"
next: "02-kappa"
---

# Kabuto

> "Seu bichinho é adorável, criança. Mas não me serve como montaria. Preciso de um maior."
> 
> — Zeebernkai, sulfure guerreiro

![Ilustração de Kabuto](../../_imagens/ameacas-arton/kabuto.jpg)
*Descrição: Um olhar apressado sugere que a grande besta-inseto seja alguma aberração da Tormenta. No entanto, o robusto besouro negro com chifres enormes age como um animal natural. Sua carapaça brilha com reflexos metálicos, e sua galhada imponente se projeta para frente como uma arma letal. Três tamanhos são mostrados: um minúsculo do tamanho de um rato, um pequeno como um cão, e um grande como um rinoceronte.*

Um olhar apressado sugere que a grande besta-inseto seja alguma aberração da Tormenta. No entanto, por estranho que pareça, o robusto besouro negro com chifres enormes age como um animal natural.

Estes grandes besouros não apenas são parte da fauna nativa de Tamu-ra, mas também são muito amados por seu povo. Tanto que alguns começam a ser trazidos para o Reinado.

Existem kabuto de tamanhos e formatos variados. Em comum, todos possuem impressionantes galhadas usadas como chifres em investidas, ou como pinças para agarrar e cortar. Esse armamento terrível e o corpo blindado podem assustar, mas na verdade os bichos são herbívoros pacíficos, lutando apenas em autodefesa. Podem ser facilmente domesticados para uma variedade de tarefas; embora não sejam muito espertos, aprendem a reconhecer o dono e obedecer a comandos simples.

Na natureza, os kabuto duelam por liderança ou fêmeas. Embora impressionantes de ver, tais embates raramente terminam em ferimentos sérios, cada lutador apenas usando a galhada para derrubar o adversário de costas. Todos os kabuto podem voar, mas de modo lento e desajeitado.

**Chibi-Kabuto:** Os menores kabuto, do tamanho de ratos, são populares como bichos de estimação ou familiares. Crianças de Tamu-ra colocam seus besouros para lutar em "torneios". No entanto, eles também podem ser treinados (ou encantados) para entrar em lugares protegidos e roubar itens, ou assassinar vítimas adormecidas com veneno ministrado em suas pinças.

**Ko-Kabuto:** Aqueles um pouco maiores são ensinados a proteger plantações contra pragas e invasores, como cães de guarda. Podem ser perigosos em estado selvagem, atacando em bandos.

**Dai-Kabuto:** Quanto aos maiores, grandes e massivos como rinocerontes, são muito apreciados como bestas de carga e montaria. Derrubam ou imobilizam um oponente com as poderosas mandíbulas, deixando-o vulnerável aos ataques do cavaleiro.

---

## Chibi-Kabuto

Besouros minúsculos usados como familiares.

### Ficha de Jogo

**ND 1/4**

**Animal Minúsculo**

| Atributo | Valor |
|----------|-------|
| **Iniciativa** | +4 |
| **Percepção** | +6 |
| **Especiais** | Faro, visão na penumbra |

| Defesa | Valor |
|--------|-------|
| **Defesa** | 13 |
| **Fortitude** | +2 |
| **Reflexos** | +3 |
| **Vontade** | –2 |

| Pontos de Vida | Valor |
|----------------|-------|
| **PV** | 8 |

| Deslocamento | Valor |
|--------------|-------|
| **Caminhando** | 6m (4q) |
| **Voo** | 3m (2q) |

**Ataques**

**Corpo a Corpo**
- Galhada +4 (1d6+2 perfuração)

**Atributos**

| For | Des | Con | Int | Sab | Car |
|-----|-----|-----|-----|-----|-----|
| –2 | 2 | 0 | –4 | 1 | –3 |

**Tesouro:** Nenhum.

**Familiar:** Um chibi-kabuto familiar aumenta em +1 o bônus na Defesa que você recebe por suas magias.

---

## Ko-Kabuto

Besouros pequenos usados como guardas ou parceiros.

### Ficha de Jogo

**ND 1**

**Animal Pequeno**

| Atributo | Valor |
|----------|-------|
| **Iniciativa** | +4 |
| **Percepção** | +6 |
| **Especiais** | Faro, visão na penumbra |

| Defesa | Valor |
|--------|-------|
| **Defesa** | 17 |
| **Fortitude** | +7 |
| **Reflexos** | +11 |
| **Vontade** | +0 |

| Pontos de Vida | Valor |
|----------------|-------|
| **PV** | 38 |

| Deslocamento | Valor |
|--------------|-------|
| **Caminhando** | 6m (4q) |
| **Voo** | 3m (2q) |

**Ataques**

**Corpo a Corpo**
- Galhada +8 (1d8+9 perfuração)

**Habilidades Especiais**

- **Agarralhada** O ko-kabuto recebe +2 em testes para agarrar e derrubar.

**Atributos**

| For | Des | Con | Int | Sab | Car |
|-----|-----|-----|-----|-----|-----|
| 0 | 2 | 1 | –4 | 1 | –3 |

**Tesouro:** Nenhum.

**Parceiro:** O ko-kabuto é um parceiro especial (guardião) que fornece os benefícios a seguir:
- **Iniciante:** Você recebe visão na penumbra e +1 na Defesa.
- **Veterano:** Uma vez por rodada, você recebe +1d8 em uma rolagem de dano corpo a corpo.
- **Mestre:** O bônus na Defesa muda para +2 e o bônus em rolagens de dano muda para +1d10.

Alternativamente, um ko-kabuto pode ser uma montaria Pequena (adequada a criaturas Minúsculas) com as estatísticas de um dai-kabuto.

---

## Horda de Ko-Kabuto

Bandos perigosos de besouros em estado selvagem.

### Ficha de Jogo

**ND 3**

**Animal Médio**

| Atributo | Valor |
|----------|-------|
| **Iniciativa** | +5 |
| **Percepção** | +7 |
| **Especiais** | Faro, visão na penumbra |

| Defesa | Valor |
|--------|-------|
| **Defesa** | 22 |
| **Fortitude** | +11 |
| **Reflexos** | +13 |
| **Vontade** | +3 |

| Pontos de Vida | Valor |
|----------------|-------|
| **PV** | 112 |

| Deslocamento | Valor |
|--------------|-------|
| **Caminhando** | 6m (4q) |
| **Voo** | 3m (2q) |

**Ataques**

**Corpo a Corpo**
- [Bando] Galhada +12 (2d6+10 perfuração)

**Habilidades Especiais**

- **Agarralhada** A horda de ko-kabuto recebe +2 em testes para agarrar e derrubar.

- **Formação Defensiva (Movimento)** A horda recebe +2 na Defesa até seu próximo turno.

**Atributos**

| For | Des | Con | Int | Sab | Car |
|-----|-----|-----|-----|-----|-----|
| 1 | 2 | 1 | –4 | 1 | –3 |

**Tesouro:** Nenhum.

---

## Dai-Kabuto

Besouros grandes usados como montarias.

### Ficha de Jogo

**ND 4**

**Animal Grande**

| Atributo | Valor |
|----------|-------|
| **Iniciativa** | +5 |
| **Percepção** | +8 |
| **Especiais** | Faro, visão na penumbra |

| Defesa | Valor |
|--------|-------|
| **Defesa** | 25 |
| **Fortitude** | +16 |
| **Reflexos** | +10 |
| **Vontade** | +4 |
| **Especiais** | Redução de dano 2 |

| Pontos de Vida | Valor |
|----------------|-------|
| **PV** | 155 |

| Deslocamento | Valor |
|--------------|-------|
| **Caminhando** | 9m (6q) |
| **Voo** | 6m (4q) |

**Ataques**

**Corpo a Corpo**
- Galhada +16 (2d8+12 perfuração, x3)

**Habilidades Especiais**

- **Agarralhada** O dai-kabuto recebe +2 em testes para agarrar e derrubar.

- **Agarrar Aprimorado (Livre)** Galhada (teste +20).

**Atributos**

| For | Des | Con | Int | Sab | Car |
|-----|-----|-----|-----|-----|-----|
| 3 | 1 | 2 | –4 | 1 | –3 |

**Tesouro:** Nenhum.

**Parceiro:** O dai-kabuto é um parceiro montaria (Grande) que fornece os benefícios a seguir:
- **Iniciante:** Seu deslocamento muda para 9m e você recebe +2 em testes de agarrar e derrubar e uma ação de movimento extra por turno (apenas para se deslocar).
- **Veterano:** Você recebe deslocamento de voo 6m.
- **Mestre:** O bônus em agarrar se aplica a todas as manobras e você recebe +2 na Defesa.

---

**Navegação:**
- [← Império de Jade](README.md)
- [Kappa →](02-kappa.md)
- [↑ Capítulo 1](../README.md)
