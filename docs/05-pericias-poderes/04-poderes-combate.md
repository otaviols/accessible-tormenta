# Poderes de Combate

> "Poderes que melhoram características relacionadas a combate."

[← Anterior: Sistema de Poderes](03-poderes-gerais-sistema.md) | [Índice](README.md) | [Próximo: Poderes de Destino →](05-poderes-destino.md)

---

## Tabela de Referência Rápida

| Poder | Pré-requisitos |
|-------|----------------|
| **Acuidade com Arma** | Des 1 |
| **Arma Secundária Grande** | Estilo de Duas Armas |
| **Arremesso Múltiplo** | Des 1, Estilo de Arremesso |
| **Arremesso Potente** | For 1, Estilo de Arremesso |
| **Ataque com Escudo** | Estilo de Arma e Escudo |
| **Ataque Pesado** | Estilo de Duas Mãos |
| **Ataque Poderoso** | For 1 |
| **Ataque Preciso** | Estilo de Uma Arma |
| **Bloqueio com Escudo** | Estilo de Arma e Escudo |
| **Carga de Cavalaria** | Ginete |
| **Combate Defensivo** | Int 1 |
| **Derrubar Aprimorado** | Combate Defensivo |
| **Desarmar Aprimorado** | Combate Defensivo |
| **Disparo Preciso** | Estilo de Disparo ou Estilo de Arremesso |
| **Disparo Rápido** | Des 1, Estilo de Disparo |
| **Empunhadura Poderosa** | For 3 |
| **Encouraçado** | Proficiência com armaduras pesadas |
| **Esquiva** | Des 1 |
| **Estilo de Arma e Escudo** | Proficiência com escudos, treinado em Luta |
| **Estilo de Arremesso** | Treinado em Pontaria |
| **Estilo de Disparo** | Treinado em Pontaria |
| **Estilo de Duas Armas** | Des 2, treinado em Luta |
| **Estilo de Duas Mãos** | For 2, treinado em Luta |
| **Estilo de Uma Arma** | Treinado em Luta |
| **Estilo Desarmado** | Treinado em Luta |
| **Fanático** | 12º nível de personagem, Encouraçado |
| **Finta Aprimorada** | Treinado em Enganação |
| **Foco em Arma** | Proficiência com a arma |
| **Ginete** | Treinado em Cavalgar |
| **Inexpugnável** | 6º nível de personagem, Encouraçado |
| **Mira Apurada** | Sab 1, Disparo Preciso |
| **Piqueiro** | — |
| **Presença Aterradora** | Treinado em Intimidação |
| **Proficiência** | — |
| **Quebrar Aprimorado** | Ataque Poderoso |
| **Reflexos de Combate** | Des 1 |
| **Saque Rápido** | Treinado em Iniciativa |
| **Trespassar** | Ataque Poderoso |
| **Vitalidade** | Con 1 |

---

## Poderes - Descrição Completa

### Acuidade com Arma
**Pré-requisito:** Des 1

Quando usa uma **arma corpo a corpo leve** ou uma **arma de arremesso**, você pode usar sua **Destreza em vez de Força** nos testes de ataque e rolagens de dano.

**Uso típico:** Espadachins ágeis, duelistas, ladinos.

---

### Arma Secundária Grande
**Pré-requisito:** Estilo de Duas Armas

Você pode empunhar **duas armas de uma mão** com o poder Estilo de Duas Armas.

**Normalmente:** Estilo de Duas Armas exige pelo menos uma arma leve.

**Com este poder:** Você pode usar duas armas de uma mão (como duas espadas longas).

---

### Arremesso Múltiplo
**Pré-requisitos:** Des 1, Estilo de Arremesso

**Uma vez por rodada**, quando faz um ataque com uma arma de arremesso, você pode gastar **1 PM** para fazer um **ataque adicional** contra o mesmo alvo, arremessando outra arma de arremesso.

**Uso típico:** Especialista em facas arremessadas, ninja.

---

### Arremesso Potente
**Pré-requisitos:** For 1, Estilo de Arremesso

Quando usa uma arma de arremesso, você pode usar sua **Força em vez de Destreza** nos testes de ataque.

**Bônus:** Se você possuir o poder **Ataque Poderoso**, poderá usá-lo com armas de arremesso.

**Uso típico:** Bárbaro que arremessa machados, guerreiro que usa azagaias.

---

### Ataque com Escudo
**Pré-requisito:** Estilo de Arma e Escudo

**Uma vez por rodada**, se estiver empunhando um escudo e fizer a ação agredir, você pode gastar **1 PM** para fazer um **ataque corpo a corpo extra com o escudo**.

**Importante:** Este ataque **não faz você perder** o bônus do escudo na Defesa.

**Uso típico:** Guerreiros defensivos que também atacam.

---

### Ataque Pesado
**Pré-requisito:** Estilo de Duas Mãos

Quando faz um ataque corpo a corpo com uma **arma de duas mãos**, você pode pagar **1 PM**.

**Se acertar:** Além do dano, você faz uma **manobra derrubar ou empurrar** contra o alvo como uma **ação livre** (use o resultado do ataque como o teste de manobra).

**Uso típico:** Guerreiros com martelos, machados grandes.

---

### Ataque Poderoso
**Pré-requisito:** For 1

Sempre que faz um ataque corpo a corpo, você pode sofrer **–2 no teste de ataque** para receber **+5 na rolagem de dano**.

**Importante:** A decisão deve ser feita **antes** de rolar o ataque.

**Uso típico:** Guerreiros de força, bárbaros.

---

### Ataque Preciso
**Pré-requisito:** Estilo de Uma Arma

Se estiver empunhando uma arma corpo a corpo em uma das mãos e **nada na outra**, você recebe:
- **+2 na margem de ameaça**
- **+1 no multiplicador de crítico**

**Exemplo:** Espada longa (19-20/x2) vira (17-20/x3).

**Uso típico:** Duelistas, espadachins refinados.

---

### Bloqueio com Escudo
**Pré-requisito:** Estilo de Arma e Escudo

Quando sofre dano, você pode gastar **1 PM** para receber **redução de dano** igual ao bônus na Defesa que seu escudo fornece contra este dano.

**Exemplo:** Escudo pesado (+3 Defesa) fornece **RD 3** contra um ataque.

**Importante:** Você só pode usar este poder **se estiver usando um escudo**.

**Uso típico:** Tanques defensivos, paladinos.

---

### Carga de Cavalaria
**Pré-requisito:** Ginete

Quando faz uma **investida montada**, você causa **+2d8 pontos de dano**.

**Bônus:** Pode **continuar se movendo** depois do ataque. Você deve se mover em linha reta e seu movimento máximo ainda é o dobro do seu deslocamento.

**Uso típico:** Cavaleiros, guerreiros montados.

---

### Combate Defensivo
**Pré-requisito:** Int 1

Quando usa a ação agredir, você pode usar este poder.

**Efeito:** Até seu próximo turno:
- Sofre **–2 em todos os testes de ataque**
- Recebe **+5 na Defesa**

**Uso típico:** Lutadores táticos, guerreiros inteligentes.

---

### Derrubar Aprimorado
**Pré-requisito:** Combate Defensivo

Você recebe **+2 em testes de ataque** para derrubar.

**Bônus:** Quando derruba uma criatura com essa manobra, pode gastar **1 PM** para fazer um **ataque extra** contra ela.

**Uso típico:** Lutadores de solo, guerreiros táticos.

---

### Desarmar Aprimorado
**Pré-requisito:** Combate Defensivo

Você recebe **+2 em testes de ataque** para desarmar.

**Bônus:** Quando desarma uma criatura, pode gastar **1 PM** para **arremessar a arma dela para longe**.

**Onde a arma cai?**
- Role **1d8** para a direção (sendo "1" diretamente à sua frente, "2" à frente e à direita, e assim por diante)
- Role **1d6** para a distância (medida em quadrados de 1,5m a partir da criatura desarmada)

**Uso típico:** Duelistas, guerreiros táticos.

---

### Disparo Preciso
**Pré-requisito:** Estilo de Disparo ou Estilo de Arremesso

Você pode fazer **ataques à distância** contra oponentes **envolvidos em combate corpo a corpo** sem sofrer a penalidade de **–5** no teste de ataque.

**Uso típico:** Arqueiros, arremessadores.

---

### Disparo Rápido
**Pré-requisitos:** Des 1, Estilo de Disparo

Se estiver empunhando uma arma de disparo que possa **recarregar como ação livre** e gastar uma **ação completa** para agredir, pode fazer um **ataque adicional** com ela.

**Custo:** Se fizer isso, sofre **–2 em todos os testes de ataque** até o seu próximo turno.

**Uso típico:** Arqueiros experientes.

---

### Empunhadura Poderosa
**Pré-requisito:** For 3

Ao usar uma arma feita para uma **categoria de tamanho maior** que a sua, a penalidade que você sofre nos testes de ataque diminui para **–2**.

**Normalmente:** Usar uma arma de uma categoria de tamanho maior impõe **–5** nos testes de ataque.

**Uso típico:** Guerreiros de força brutal, bárbaro gigante.

---

### Encouraçado
**Pré-requisito:** Proficiência com armaduras pesadas

Se estiver usando uma **armadura pesada**, você recebe **+2 na Defesa**.

**Bônus Escalável:** Esse bônus aumenta em **+2** para cada outro poder que você possua que tenha Encouraçado como pré-requisito.

**Poderes que contam:**
- **Inexpugnável:** +2 adicional (total +4)
- **Fanático:** +2 adicional (total +6)

**Uso típico:** Tanques pesados, guerreiros de linha de frente.

---

### Esquiva
**Pré-requisito:** Des 1

Você recebe **+2 na Defesa** e **+2 em Reflexos**.

**Uso típico:** Personagens ágeis, evitadores.

---

### Estilo de Arma e Escudo
**Pré-requisitos:** Proficiência com escudos, treinado em Luta

Se você estiver usando um **escudo**, o bônus na Defesa que ele fornece **aumenta em +2**.

**Exemplo:** Escudo leve (+2) vira +4, escudo pesado (+3) vira +5.

**Uso típico:** Guerreiros defensivos.

---

### Estilo de Arremesso
**Pré-requisito:** Treinado em Pontaria

Você pode **sacar armas de arremesso** como uma **ação livre** e recebe **+2 nas rolagens de dano** com elas.

**Bônus:** Se também possuir o poder **Saque Rápido**, também recebe **+2 nos testes de ataque** com essas armas.

**Uso típico:** Especialistas em facas, machados arremessados.

---

### Estilo de Disparo
**Pré-requisito:** Treinado em Pontaria

Se estiver usando uma **arma de disparo**, você soma sua **Destreza nas rolagens de dano**.

**Uso típico:** Arqueiros, besteiros.

---

### Estilo de Duas Armas
**Pré-requisitos:** Des 2, treinado em Luta

Se estiver empunhando **duas armas** (e pelo menos uma delas for leve) e fizer a ação agredir, você pode fazer **dois ataques**, um com cada arma.

**Custo:** Se fizer isso, sofre **–2 em todos os testes de ataque** até o seu próximo turno.

**Bônus:** Se possuir **Ambidestria** (raça qareen), em vez disso **não sofre penalidade** para usá-lo.

**Uso típico:** Ladinos, rangers, duelistas de duas armas.

---

### Estilo de Duas Mãos
**Pré-requisitos:** For 2, treinado em Luta

Se estiver usando uma arma corpo a corpo com **as duas mãos**, você recebe **+5 nas rolagens de dano**.

**Importante:** Este poder não pode ser usado com armas leves.

**Uso típico:** Guerreiros de armas grandes, bárbaros.

---

### Estilo de Uma Arma
**Pré-requisito:** Treinado em Luta

Se estiver usando uma arma corpo a corpo em uma das mãos e **nada na outra**, você recebe:
- **+2 na Defesa**
- **+2 nos testes de ataque** com essa arma (exceto ataques desarmados)

**Uso típico:** Duelistas clássicos.

---

### Estilo Desarmado
**Pré-requisito:** Treinado em Luta

Seus **ataques desarmados** causam **1d6 pontos de dano** e podem causar **dano letal ou não letal** (sem penalidades).

**Uso típico:** Monges, lutadores, bárbaros tribais.

---

### Fanático
**Pré-requisitos:** 12º nível de personagem, Encouraçado

Seu deslocamento **não é reduzido** por usar armaduras pesadas.

**Normalmente:** Armaduras pesadas reduzem o deslocamento.

**Bônus:** Também conta para o bônus escalar de **Encouraçado** (+2 Defesa adicional).

**Uso típico:** Campeões de armadura completa.

---

### Finta Aprimorada
**Pré-requisito:** Treinado em Enganação

Você recebe **+2 em testes de Enganação** para fintar e pode fintar como uma **ação de movimento** (em vez de ação padrão).

**Uso típico:** Ladinos, duelistas astuciosos.

---

### Foco em Arma
**Pré-requisito:** Proficiência com a arma

Escolha uma arma. Você recebe **+2 em testes de ataque** com essa arma.

**Múltiplas Escolhas:** Você pode escolher este poder outras vezes para armas diferentes.

**Uso típico:** Especialistas em uma arma específica.

---

### Ginete
**Pré-requisito:** Treinado em Cavalgar

Você passa **automaticamente** em testes de Cavalgar para não cair da montaria quando sofre dano.

**Bônus:** Não sofre penalidades para **atacar à distância** ou **lançar magias** quando montado.

**Uso típico:** Cavaleiros, arqueiros montados.

---

### Inexpugnável
**Pré-requisitos:** 6º nível de personagem, Encouraçado

Se estiver usando uma **armadura pesada**, você recebe **+2 em todos os testes de resistência** (Fortitude, Reflexos, Vontade).

**Bônus:** Também conta para o bônus escalar de **Encouraçado** (+2 Defesa adicional).

**Uso típico:** Tanques supremos, paladinos.

---

### Mira Apurada
**Pré-requisitos:** Sab 1, Disparo Preciso

Quando usa a **ação mirar**, você recebe:
- **+2 em testes de ataque** à distância
- **+2 na margem de ameaça** com ataques à distância

**Duração:** Até o fim do turno.

**Uso típico:** Atiradores de elite, snipers.

---

### Piqueiro
**Pré-requisito:** —

Se estiver empunhando uma **arma alongada**, **uma vez por rodada**, sempre que um inimigo entrar **voluntariamente** em seu alcance corpo a corpo, você pode gastar **1 PM** para fazer um **ataque corpo a corpo** contra este oponente com esta arma.

**Bônus:** Se o oponente tiver se aproximado fazendo uma **investida**, seu ataque causa **dois dados de dano extra** do mesmo tipo.

**Exemplo:** Lança (1d8) causa 3d8 contra investida.

**Uso típico:** Guardas, defensores de choke points.

---

### Presença Aterradora
**Pré-requisito:** Treinado em Intimidação

Você pode gastar uma **ação padrão** e **1 PM** para **assustar** todas as criaturas à sua escolha em **alcance curto**.

**Regras:** Veja a perícia **Intimidação** para as regras de assustar.

**Recordando:** Teste oposto de Intimidação vs Vontade, abalado ou apavorado.

**Uso típico:** Guerreiros intimidadores, líderes aterrorizantes.

---

### Proficiência
**Pré-requisito:** —

Escolha uma proficiência:
- Armas marciais
- Armas de fogo
- Armaduras pesadas
- Escudos
- Armas exóticas (se já for proficiente em armas marciais)

Você recebe essa proficiência.

**Múltiplas Escolhas:** Você pode escolher este poder outras vezes para proficiências diferentes.

**Uso típico:** Qualquer personagem querendo expandir opções de equipamento.

---

### Quebrar Aprimorado
**Pré-requisito:** Ataque Poderoso

Você recebe **+2 em testes de ataque** para quebrar.

**Bônus:** Quando reduz os PV de uma arma para **0 ou menos**, você pode gastar **1 PM** para realizar um **ataque extra contra o usuário dela**.

**Detalhe:** O ataque adicional usa os mesmos valores de ataque e dano, mas os dados devem ser **rolados novamente**.

**Uso típico:** Destruidores de equipamento, quebradores.

---

### Reflexos de Combate
**Pré-requisito:** Des 1

Você ganha uma **ação de movimento extra** no seu **primeiro turno** de cada combate.

**Uso típico:** Personagens ágeis que querem agir primeiro.

---

### Saque Rápido
**Pré-requisito:** Treinado em Iniciativa

Você recebe **+2 em Iniciativa** e pode **sacar ou guardar itens** como uma **ação livre** (em vez de ação de movimento).

**Bônus:** A ação que você gasta para **recarregar armas de disparo** diminui em **uma categoria**:
- Ação completa → ação padrão
- Ação padrão → ação de movimento
- Ação de movimento → ação livre

**Uso típico:** Atiradores, personagens que trocam de armas frequentemente.

---

### Trespassar
**Pré-requisito:** Ataque Poderoso

Quando você faz um ataque corpo a corpo e reduz os pontos de vida do alvo para **0 ou menos**, pode gastar **1 PM** para fazer um **ataque adicional** contra outra criatura **dentro do seu alcance**.

**Uso típico:** Guerreiros cleave, destruidores em massa.

---

### Vitalidade
**Pré-requisito:** Con 1

Você recebe:
- **+1 PV por nível de personagem**
- **+2 em Fortitude**

**Exemplo:** Personagem de 5º nível ganha +5 PV imediatamente.

**Uso típico:** Personagens que querem mais sobrevivência.

---

## Cadeias de Poderes Comuns

### Cadeia do Escudo
1. **Estilo de Arma e Escudo** (base)
2. **Ataque com Escudo** (ofensivo)
3. **Bloqueio com Escudo** (defensivo)

### Cadeia da Armadura Pesada
1. **Encouraçado** (base, +2 Defesa)
2. **Inexpugnável** (6º nível, +2 resistências, +2 Defesa total)
3. **Fanático** (12º nível, sem redução deslocamento, +2 Defesa total)

**Total final:** +6 Defesa, +2 todas resistências, sem penalidade deslocamento!

### Cadeia do Combate Tático
1. **Combate Defensivo** (base)
2. **Derrubar Aprimorado** (especialização derrubar)
3. **Desarmar Aprimorado** (especialização desarmar)

### Cadeia do Arqueiro
1. **Estilo de Disparo** (base, Des no dano)
2. **Disparo Preciso** (atirar em combate corpo a corpo)
3. **Mira Apurada** (bônus ao mirar)
4. **Disparo Rápido** (ataque extra)

### Cadeia do Ataque Poderoso
1. **Ataque Poderoso** (base, –2 ataque/+5 dano)
2. **Quebrar Aprimorado** (destruir equipamento)
3. **Trespassar** (matar múltiplos inimigos)

---

## Dicas de Uso

### Para Guerreiros

**Tanque Defensivo:**
- Estilo de Arma e Escudo → Bloqueio com Escudo
- Encouraçado → Inexpugnável → Fanático
- Vitalidade

**Destruidor Ofensivo:**
- Estilo de Duas Mãos → Ataque Pesado
- Ataque Poderoso → Trespassar
- Foco em Arma (arma favorita)

**Duelista Técnico:**
- Estilo de Uma Arma → Ataque Preciso
- Combate Defensivo → Finta Aprimorada
- Acuidade com Arma (se Des alto)

### Para Lutadores/Monges
- Estilo Desarmado (base essencial)
- Combate Defensivo → Derrubar Aprimorado
- Reflexos de Combate
- Esquiva

### Para Arqueiros
- Estilo de Disparo (essencial)
- Disparo Preciso (essencial)
- Foco em Arma (arco/besta)
- Disparo Rápido (nível alto)

### Para Cavaleiros
- Ginete (essencial)
- Carga de Cavalaria
- Estilo de Arma e Escudo (defensivo)
- ou Estilo de Uma Arma (ofensivo)

---

## Resumo por Conceito

### Defensores
Encouraçado, Inexpugnável, Fanático, Estilo de Arma e Escudo, Bloqueio com Escudo, Vitalidade

### Destruidores
Ataque Poderoso, Estilo de Duas Mãos, Trespassar, Foco em Arma, Ataque Pesado

### Duelistas
Estilo de Uma Arma, Ataque Preciso, Finta Aprimorada, Acuidade com Arma, Esquiva

### Táticos
Combate Defensivo, Derrubar Aprimorado, Desarmar Aprimorado, Quebrar Aprimorado

### Atiradores
Estilo de Disparo, Disparo Preciso, Disparo Rápido, Mira Apurada, Saque Rápido

### Dois Armas
Estilo de Duas Armas, Arma Secundária Grande, Acuidade com Arma

### Cavaleiros
Ginete, Carga de Cavalaria, Proficiência, Reflexos de Combate

---

[← Anterior: Sistema de Poderes](03-poderes-gerais-sistema.md) | [Índice](README.md) | [Próximo: Poderes de Destino →](05-poderes-destino.md)
